...
test('Verify address details in checkout page',async({page})=>{
    ...
    await page.locator('form').filter({ hasText: 'Signup' }).getByPlaceholder('Email Address').fill('allen3123@gmail.com');
    ...
    await expect(page.getByText('#form > div > div > div > h2 > b')).toBeVisible();
    await page.locator('#form > div > div > div > div > a').click();
